...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 1100011", blt      , B, s->dnpc = (src1 < src2)?imm+s->pc:s->snpc);
  INSTPAT("??????? ????? ????? 101 ????? 1100011", bge      , B, s->dnpc = (src1 >= src2)?imm+s->pc:s->snpc);
  ...
  INSTPAT("??????? ????? ????? 100 ????? 0000011", lbu      , I, R(rd) = Mr(src1 + imm, 1)&0x000000FF);
  INSTPAT("??????? ????? ????? 101 ????? 0000011", lhu      , I, R(rd) = Mr(src1 + imm, 2)&0x0000FFFF);
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 0010011", slli     , I, R(rd) = (uint32_t)src1 << imm);
  INSTPAT("0000000 ????? ????? 101 ????? 0010011", srli     , I, R(rd) = (uint32_t)src1 >> imm);
  INSTPAT("0100000 ????? ????? 101 ????? 0010011", srai     , I, R(rd) =  (int32_t)src1 >> imm);
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 0110011", srl      , R, R(rd) = (uint32_t)src1 << (src2&0b11111));
  INSTPAT("0100000 ????? ????? 101 ????? 0110011", sra      , R, R(rd) = (uint32_t)src1 >> (src2&0b11111));
