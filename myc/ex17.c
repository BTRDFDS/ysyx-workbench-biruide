...
void dataFind(struct Connection *conn, int idFind, char* nameFind, char* emailFind)
{
    if(conn==NULL){die(conn,"errF0");}
    if(conn->db==NULL){die(conn,"errF1");}
    struct Database *db = conn->db;
    int i = 0;
    int nameMatch = 0;
    int emailMatch = 0;
    for(i = 0; i < MAX_ROWS; i++) {
        ...
        if(cur->set) {
            ...
            if(nameFind!=NULL){
            nameMatch = (nameFind && strstr(cur->name, nameFind) != NULL);}
            if(emailFind!=NULL){
            emailMatch = (emailFind && strstr(cur->email, emailFind) != NULL);}
...
int main(int argc, char *argv[])
{
    ...
    switch(action) {
        ...
        case 'l':
            ...
            break;
        case 'f':
            dataFind(conn, id, argv[4], argv[5]);
            break;
        default:
