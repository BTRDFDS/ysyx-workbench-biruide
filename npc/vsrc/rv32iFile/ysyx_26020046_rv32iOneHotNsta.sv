module ysyx_26020046_rv32iOneHotNsta(clk,reset,code,pc,stop,eb,pmem_read,pmem_write,addr,mask,enW);
...
module ysyx_26020046_rv32iIDC(clk,code,reset,enJfun,enMret,enEcall,imm,cRd,cR1,cR2,op,enCsr,csrAddr,stop,eb);
	...
	always_ff @(posedge clk) begin
		if(~reset)begin
			if(op.Csrr.Ebreak)begin stop=1;eb=1;end
			else if(~((|op.Iner)|(|op.Csrr)))begin stop=1;eb=1;end
			else begin stop=0;eb=0;end
		end
	end
...
module ysyx_26020046_rv32iLSU(clk,reset,addr,oR2,enBfun,enJfun,op,data,pc,enMret,enEcall,pmem_read,mask);
...
`endif
	...
	assign iRAM = (|op.Lfun)&clk?pmem_read:0;
