...
class ysyx_26020046_WBU(val Width:Int=32,val RegNum:Int=32,val CsrWidth:Int=12,val PcReset:Int=0x80000000) extends Module {
	...
	val errRead = Wire(Bool)
	val errWrite = Wire(Bool)

	errWrite := 0.U
	when(io.waterLsWb.valid){
		when(io.waterLsWb.rdAddr =/= 0.U){gpr(io.waterLsWb.rdAddr) := io.waterLsWb.result}
	...
	switch(io.immWbLs.csrAddr){
		...
		otherwise{errRead := 1.U;io.immWbLs.csrOut := 0.U}
	}

	io.immWbLs.ready:= 1.U
	io.immWbLs.wash	:= errRead | errWrite
	io.immWbLs.addr	:= Mux(errRead|errWrite,mtvec,0.U)
}