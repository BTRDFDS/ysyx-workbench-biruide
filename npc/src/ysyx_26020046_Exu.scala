...
class ysyx_26020046_Exu(val Yosys:Boolean=false) extends Module {
	...
	val pipeFenceI	= PipeReg(false.B	,false.B			,out.imme.ready,in.pipe.fenceI	)
	val pipeJump	= PipeReg(false.B	,false.B			,out.imme.ready,in.pipe.jump	)
	val pipeUpdate	= PipeReg(false.B	,IfuUpdate.Null		,out.imme.ready,in.pipe.update	)
	val pipeRdAddr	= PipeReg(false.B	,0.U(RegWidth.W)	,out.imme.ready,in.pipe.rdAddr	)
	val pipeResult	= PipeReg(false.B	,0.U(BitWidth.W)	,out.imme.ready,in.pipe.result	)
	val pipePc		= PipeReg(false.B	,0.U((BitWidth-2).W),out.imme.ready,in.pipe.pc		)
	val pipeCsrAddr	= PipeReg(false.B	,0.U(CsrWidth.W)	,out.imme.ready,in.pipe.csrAddr	)
	val pipeCsrMesg	= PipeReg(false.B	,0.U(BitWidth.W)	,out.imme.ready,in.pipe.csrMesg	)
	val pipeR1		= PipeReg(false.B	,0.U(BitWidth.W)	,out.imme.ready,in.pipe.r1		)
	val pipeR2		= PipeReg(false.B	,0.U(BitWidth.W)	,out.imme.ready,in.pipe.r2		)
	val pipeLsuOp	= PipeReg(false.B	,LsuOp.Null			,out.imme.ready,in.pipe.lsuOp	)
	val pipeLsuAddr	= PipeReg(false.B	,LsuAddr.B			,out.imme.ready,in.pipe.lsuAddr	)
	val pipeCsrOp	= PipeReg(pipeReset	,CsrOp.Null			,out.imme.ready,in.pipe.csrOp	)
	val pipeAlu		= PipeReg(false.B	,ExuAlu.Null		,out.imme.ready,in.pipe.alu		)
	val pipeBfu		= PipeReg(false.B	,ExuBfu.Null		,out.imme.ready,in.pipe.bfu		)
	val pipeCsr		= PipeReg(false.B	,ExuCsr.Null		,out.imme.ready,in.pipe.csr		)
	val pipeRes		= PipeReg(false.B	,ExuRes.Alu			,out.imme.ready,in.pipe.res		)
	val pipeIn1		= PipeReg(false.B	,ExuIn1.R1			,out.imme.ready,in.pipe.in1		)
	val pipeIn2		= PipeReg(false.B	,ExuIn2.R2			,out.imme.ready,in.pipe.in2		)
