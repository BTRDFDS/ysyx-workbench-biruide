...
class ysyx_26020046_Exu(val Yosys:Boolean=false) extends Module {
	...
	//BJR
	// out.imme.btbb := ~in.imme.error && (pipeValid && (pipeBfu=/=ExuBfu.Null || pipeEnJcod))
	...
	out.imme.btbb := ~in.imme.error && (pipeValid && (pipeBfu=/=ExuBfu.Null || (pipeEnJcod && pipeAlu=/=ExuAlu.Jalr)))
