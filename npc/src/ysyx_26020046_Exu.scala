...
class ysyx_26020046_Exu(val Yosys:Boolean=false) extends Module {
	...
	//BJR
	out.imme.jbpu := (~in.imme.error && pipeValid) && (pipeEnBpu)
	out.imme.jump :=  in.imme.error || (pipeValid && (pipeEnBpu || pipeEnJcod)&& ~hasSend && shouldBe=/=Cat(in.pipe.pc,0.U(2.W)))//shouldBe其实还需要out.imme.jbpu，但是这里未进行拆分因此可以直接这样子
	...
	if(Yosys == false){
		...
		exuChk.jum	:= pipeValid&&pipeBfu===ExuBfu.Null&& out.imme.jump && pipeEnJcod && pipeAlu=/=ExuAlu.Jalr
		exuChk.jlr	:= pipeValid&&pipeBfu===ExuBfu.Null&& out.imme.jump && pipeEnJcod && pipeAlu===ExuAlu.Jalr
