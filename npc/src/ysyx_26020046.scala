...
class ysyx_26020046(val PcInit:UInt=0x30000000L.U,val Yosys:Boolean=false) extends Module {
	...
	if(Yosys == false){
		...
		val shouldbe = Cat(Mux(Get(exu.in.pipe.valid),Get(exu.in.pipe.pc),Get(ifu.pipePc)),0.U(2.W));dontTouch(shouldbe)
		...
		when(noEbreak){
			...
			when(Get(exu.enBfun)=/=btbExu && (Get(exu.pipeBfu)=/=ExuBfu.Null)&& ~Get(exu.hasSend))	{chk.jbHit	:= true.B}
