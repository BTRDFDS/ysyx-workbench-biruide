...
const char *npcsdbRegs[32] = {//注意：0号寄存器替代为pc
  "pc", "ra",  "sp",  "gp", "tp", "t0", "t1", "t2",
  "s0", "s1",  "a0",  "a1", "a2", "a3", "a4", "a5",
  "a6", "a7",  "s2",  "s3", "s4", "s5", "s6", "s7",
  "s8", "s9", "s10", "s11", "t3", "t4", "t5", "t6"
};
uint32_t npcsdbGpr[32]={0};
...
static int cmd_info(char *args) {
	...
	case 'r':
		...
		for(int i=0;i<4;i++){
			for(int j=0;j<8;j++){
				printf("%2d:%3s=%8x ",i*8+j,npcsdbRegs[i*8+j],npcsdbGpr[i*8+j]);
			}
			printf("\n");
		}
		break;
