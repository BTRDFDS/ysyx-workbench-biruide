...
可以看到：新版的独热码系统相比旧版的,损失了一定的性能但是极大的提升了可读性。值得注意的是，我在后续进行了大量的测试，包括不限于对ALU进行拆分，在IDC中添加在ebreak时的处理……但是结果证明这些处理都不如第一版的。部分修正在面积上有所改善，但也不如旧版的，此外都无一例外的极大的降低了最低频率，甚至是降低到了6??Mhz左右，而功耗也是升多降少。在经过我对一番努力后，最终实现了极限的优化版本
