...
|综合结果	|面积um2    |面积%  |功耗W  |功耗%|频率Mhz|频率%|
|:--------:	|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|
|old		|16623	|0%     |0.48	|0%     |783	|0%     |
|onehot		|17244	|+3.7%	|0.67	|+40.6%	|768	|-1.9%	|
|onehotALU	|17393	|+4.6%	|0.57	|+18.8%	|742	|-5.2%	|
|onehotIDC	|16459	|-0.9%	|0.67	|+40.6%	|686	|-12.1%	|
|onehotAI	|17043  |+2.5%  |0.37   |-23.0% |781    |-0.25%	|
|onehotAIc	|16843  |+1.3%  |0.74   |+54.2% |815    |+4.09%	|
|onehotAI+	|16161	|-2.8%	|0.21	|-5623%	|745	|-4.85%	|
|onehotAI+c	|

可以看到：新版的独热码系统相比旧版的,损失了一定的性能但是极大的提升了可读性。值得注意的是，我在后续进行了大量的测试，包括不限于对ALU进行拆分，在IDC中添加在ebreak时的处理……但是结果证明这些处理都不如第一版的。部分修正在面积上有所改善，但也不如旧版的，此外都无一例外的极大的降低了最低频率，甚至是降低到了6??Mhz左右，而功耗也是升多降少。在经过我对一番努力后，最终实现了极限的优化版本，即A+I是ALU的拆分(把用于LSU的计算单拎出来)，以及IDC在ebreak的时候直接把寄存器控制设强制为0；而c是指代关于stop（停止，包括指令错误和ebreak）是否是时序电路（posedge clk）。最总的优化结果显著。
